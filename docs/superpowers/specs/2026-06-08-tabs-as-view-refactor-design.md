...
**Test seams (to meet the `.coverage-thresholds.json` gate — 95% — without a live tmux server):** `PureClaw.Tabs` and `PureClaw.Tabs.Relay` take an **injected deps record** (harness-liveness probe, death-event source, clock, output sink) — mirroring the existing `ReconcileDeps`/`ClaudeCodeDeps` pattern — so the relay engine, two-phase harness-death removal, and boot reconcile are driven deterministically in unit tests.

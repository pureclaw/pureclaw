...
- **Cabal registration (every Create step):** add each new `src/` module to `pureclaw.cabal` `library: exposed-modules`, and each new test spec to the `test-suite: other-modules` — the build fails with "module not found" otherwise. Do this in the same task that creates the file.
...
- [ ] **Step 3: Implement.** In `CLI/Commands.hs` where `reconcileDeps` is built (`~996`):
  ...
  3. Add `_rd_providerFor` selecting the log provider for an entry iff **`_he_flavour == HClaudeCode && a `ClaudeSessionUuid` resolves`** — gate on **uuid-resolvability, NOT origin**. (Rationale: only spawned-with-uuid harnesses persist `_h_claudeSessionUuid`; adopted/CLI/non-claude lack it, so uuid-presence is the correct capability signal — and it is origin-agnostic, so it also covers `OriginDiscovered` for the future re-attach case without a code change. **Binding caveat:** `stepTurns` only processes BOUND entries (`withBoundEntry` needs `_he_handle` + `_he_sessionId`). A harness spawned in THIS run is bound. A boot-reconstructed entry after a *process* restart is unbound (`_he_handle`/`_he_sessionId` = `Nothing`) and is skipped until the deferred detach/re-attach lifecycle binds it — so post-process-restart streaming is OUT OF SCOPE here, see Task 6 Step 3.) Resolution, done ONCE then cached per `HarnessId`: `_he_sessionId → session.json HarnessSpec._h_claudeSessionUuid → mkClaudeSessionUuid` (validate); `base <- resolveClaudeBase`; `mkSafeClaudeLogPath base uuid (fmap T.unpack (_h_canonicalCwd hs))` (the field is `Maybe Text`; the param is `Maybe FilePath`) → on `Left _` (or no uuid), the provider is `tmuxProvider`/`nullProvider` (fallback, WARN once); on `Right safePath`, seed `recordedIds` from that session's transcript (`seedRecordedIds`), start `runLogTailer` under `Async.withAsync`, store the `Async` in a sidecar `IORef (Map HarnessId (Async ()))`, and cache the resulting log `TurnProvider`. On reconcile-detected exit (reuse the `_rd_evict`/terminal signal), cancel + remove the `Async`.
...
- [ ] **Step 1: Write the integration test** (drives the real wiring with a temp `~/.claude`-style dir + a fixture `<uuid>.jsonl` for a BOUND spawned-claude harness, asserting the assistant prose lands in the session transcript exactly once). Then a **re-attach-idempotency** assertion: re-seed `recordedIds` from the now-populated transcript and re-run the tailer over the SAME file → it re-emits nothing (this models a re-started tailer / future re-attach against a transcript that already holds the turns — the realistic shape of "restart" given the binding caveat below). Do NOT attempt to drive a full process-restart boot→rebind cycle (the harness is unbound after a process restart — out of scope).
...
- [ ] **Step 3:** Manual verification (record results in the PR):
  ...
  3. **Binding caveat (document, don't test as working):** a harness does not stream after a PureClaw *process* restart until handle re-attach lands (pre-existing limitation, unbound boot-reconstructed entry); the disk-seeded dedup is forward-safe for that case and prevents duplication on any within-run tailer re-read. Post-restart re-attach is OUT OF SCOPE.
- [ ] **Step 4: Commit** `test(integration): claude-log core content source end-to-end + re-attach idempotency`.

---

## Self-review notes

- Spec coverage: bounded read/caps (Task 1,3), prose-only fold + sanitization + finalize (Task 2), tailer mechanics + seek-EOF + shrink-reset + async (Task 3), provider seam + minimal stepTurns + tmux-verbatim + latch + finalize-authority (Task 4), disk-seeded dedup + lifecycle + single-writer (Task 5), end-to-end + re-attach idempotency (Task 6). Security (SafeClaudeLogPath, O_NOFOLLOW, caps, sanitize) is satisfied by reuse + caps in Tasks 1/3/5.
- Out of scope (no tasks, intentional): thinking/tool/Request surfacing; adopted/CLI/non-claude log feed; optional opt-in view; **post-process-restart re-attach/re-binding of a harness** (pre-existing unbound-after-restart limitation; the dedup is forward-safe for it, but binding it is the separate deferred detach/re-attach lifecycle).

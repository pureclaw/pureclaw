...
- Local `-Werror` relax (first action; gitignored, transient): write `cabal.project.local` = `package pureclaw\n  ghc-options: -Wwarn`. Remove it in WU11.
- Commit trailer: `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`

**Local build relaxation (for the duration of this work):** create a **gitignored** `cabal.project.local` with `package pureclaw` / `ghc-options: -Wwarn` to demote warnings-as-errors locally. This lets the legacy and new systems **coexist during the transition** so the cutover can be done as small **migrate-first / delete-last** commits instead of one big-bang. **CI still enforces `-Werror`** (`cabal.project:12`), so the final WU restores full `-Werror` cleanliness; the work is not done until CI is green. Restore by deleting `cabal.project.local`.

**Sequencing principle (drives the whole plan):** WU1–WU7 are **purely additive** — new modules + tests; the existing tree keeps compiling and passing untouched. **WU8 is the cutover, done incrementally with `-Werror` off: migrate every consumer (source + test) onto the new model first, then delete each legacy module last (once nothing imports it).** Because deletion is always the final step for each module, no commit ever imports something that no longer exists — even under `-Werror` the *final* state is clean, and the intermediate states are clean except for tolerated transitional warnings. WU9–WU11 finish on the new model; **WU11 restores `-Werror`** and proves CI-green.
...
> **The cutover — done incrementally (migrate-first / delete-last) with `-Werror` relaxed locally.** Execute in a dedicated worktree. Order: **(1)** wire the new system into `AgentEnv`/`Dispatcher` *alongside* the legacy fields/modules (both coexist, transitional warnings tolerated); **(2)** migrate every consumer — source and test — onto the new model; **(3)** only once a legacy module/symbol has **no remaining importer**, delete it. Each sub-step is its own small commit that **builds and passes tests** (warnings allowed locally). Because deletion is always last, no commit imports a missing module. May be split into sub-WUs (8a wire, 8b migrate handlers + Agent/Loop, 8c migrate tests, 8d delete legacy) — they need not be atomic now that `-Werror` is off. **Run `cabal build` (not just `cabal test`) at each sub-commit and skim the warning output** — don't let `-Wwarn` hide a warning-class defect (incomplete pattern, `-Wincomplete-record-updates` from a new `AgentEnv` field) until WU11/CI. Commit per sub-step in the worktree for bisectability.

> **Authoritative blast radius (grep-derived — the complete set of consumers to migrate before deleting).** Every module below imports a deleted module or a removed symbol (`TabHandle`, `TabRunner`, `TabStatus(Active|Idle|Crashed)`, `mkTabAi`/`mkTabHarness`/`defaultTabFactory`, `TabUnsupportedCommand`, `_env_focus`, `_env_session`, `_env_tabs`, `_env_runners`, `dispatchLegacyTabCmd`, `startChannelOut`, `shouldEmit`, `insertTab`) — each is migrated in step (2) before the corresponding deletion in step (3):
>
...
- Delete: `src/PureClaw/Routing/{AutoSpawn,Registry,Dashboard,ChannelOut,LegacyDispatch}.hs`, `src/PureClaw/Tab/{Ai,Harness,Backend}.hs` (and their own specs), `src/PureClaw/Handles/Tab.hs-boot` (the slim dissolves the cycle — see Step 2).
- **Transitive test importers (must be owned here or the test build ends red):** delete/rewrite `test/Test/Fake/TabFactory.hs` (builds `TabHandle` via the deleted factory), `test/Coexistence/SlashCmdSpec.hs` (imports `Handles.Tab(TabHandle,TabStatus)`, `Routing.Registry`, `Test.Fake.TabFactory`, `_env_tabs`), `test/Security/TabSpec.hs` (imports `Routing.AutoSpawn`, `Routing.Registry`, `Tab.Backend`) — and drop their `pureclaw.cabal` `other-modules` entries (lines ~378, 379, 392).
...
**TDD steps:**
- [ ] **Step 1 — wire new ALONGSIDE legacy (no removal yet):** add `TabRegistry`, `CursorState` ref, `SessionPool`, lifecycle `TQueue` to `AgentEnv` *next to* the still-present `_env_focus`/`_env_session`/`_env_tabs`; make the new per-conversation dispatch + `Tabs.Relay` the live path; wire `SessionPool.acquire/release` into the AI tab loop. Legacy modules still compile (transitional unused-import warnings tolerated locally). Build + existing tests green.
- [ ] **Step 2 — migrate consumers onto the new model (legacy still present):** rewire `Agent/Loop.hs` off `dispatchLegacyTabCmd`, `SlashCommands.hs`/`Parse.hs` to flat verbs, `Routing/{Onboarding,Types}.hs`, `CLI/Commands.hs`; **migrate/rewrite every test module in the blast-radius list** onto the new model (delete pure-legacy specs; rewrite the rest). After this step nothing references `_env_focus`/`_env_session`/the legacy modules/`shouldEmit`/`insertTab`/`TabHandle`. Build + tests green.
...
- [ ] **Step 10 — delete legacy LAST (now unreferenced):** delete `Routing.{AutoSpawn,Registry,Dashboard,ChannelOut,LegacyDispatch}` + `Tab.{Ai,Harness,Backend}` + their specs + `pureclaw.cabal` entries; **slim** `Handles.Tab` — drop `TabHandle`/factory/`TabStatus`, drop `TabError`'s `TabUnsupportedCommand !SlashCommand` constructor (dissolving the `Handles.Tab → SlashCommands` cycle so **`Handles/Tab.hs-boot` is deleted**, sole importer `Env.hs:27` in scope); keep `TabKind`/slimmed `TabError`; re-export `TabIndex`. Because Steps 1–2 migrated every consumer, no import dangles. **Post-slim safety grep:** confirm the 5 surviving `Handles.Tab` importers (`Frontend/API.hs`, `Routing/{Types,Config,PromptRenderer,Parse}.hs`) still compile against the re-exported `TabIndex` + kept `TabKind`/`TabError`. `cabal build` + `cabal test` green (warnings tolerated locally; `-Werror` restored in WU11). Commit.

**DoD:** legacy modules deleted; globals removed; all routing + command + wizard-interception + tombstone + concurrency tests pass; §14 copy asserted; `cabal build` + `cabal test` green locally (full `-Werror` cleanliness deferred to WU11).
...
- [ ] **Step 3 — restore `-Werror` + clean warnings:** delete `cabal.project.local`; `cabal build` with `-Werror` back on; fix every residual warning surfaced by the transition (unused imports, incomplete patterns, missing-export-lists, `-Wincomplete-record-updates`). This is the step that pays back the temporary relaxation; CI enforces it anyway.
- [ ] **Step 4 — full gate:** `cabal test --enable-coverage` meets `.coverage-thresholds.json` (95%); `hlint src test` clean; `cabal build` `-Werror` clean.
- [ ] **Step 5 — file frontend-parity follow-up issue** in the epic, referencing #79. Commit.

**DoD:** `-Werror` restored and clean (CI-green); end-to-end chat surface green against the real binary; retired waivers removed, modified-module coverage satisfied, survivors intact; coverage/lint/build gates pass; follow-up filed.

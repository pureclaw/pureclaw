...
- [ ] **Step 2 — delete legacy + slim `Handles.Tab`:** delete `Routing.{AutoSpawn,Registry,Dashboard,ChannelOut,LegacyDispatch}` + `Tab.{Ai,Harness,Backend}` + their specs + `.cabal` entries; **slim** `Handles.Tab` — drop `TabHandle`/factory/`TabStatus`, **drop `TabError`'s `TabUnsupportedCommand !SlashCommand` constructor** (the only reason `Handles.Tab` imported `Agent.SlashCommands`), which dissolves the import cycle so **`Handles/Tab.hs-boot` is deleted** (its sole importer `Env.hs:27` is in scope); keep `TabKind`/the slimmed `TabError`; re-export `TabIndex`. Rewire `Agent/Loop.hs` off `dispatchLegacyTabCmd` onto the new dispatcher; fix dangling imports in `Dispatcher.hs:137,149-151`, `Agent/Loop.hs:37,136`, `CLI/Commands.hs`, `SlashCommands.hs`. **Own the transitive test importers:** delete/rewrite `test/Test/Fake/TabFactory.hs`, `test/Coexistence/SlashCmdSpec.hs`, `test/Security/TabSpec.hs` onto the new model and drop their `pureclaw.cabal` `other-modules` entries. Verify `Frontend/API.hs` + `Routing/{Types,Config,PromptRenderer}.hs` compile **unchanged**. Both `cabal build` AND `cabal test` (test-suite compile) must be clean here.

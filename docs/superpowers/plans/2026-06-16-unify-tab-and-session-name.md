...
- [ ] **Step 3: Implement.**
  ...
  - **`CLI/Commands.hs` builds the closure per `ServerMode`** (already in scope — the tui-requires-gateway change added it):
    ...
    - `RequireGateway` (tui process): HTTP `PUT http://127.0.0.1:<port>/api/sessions/<sid>/description` body `{"description": <name>}` via the existing `manager` (the one `probeGatewayUp` uses), mapping non-2xx / exceptions to `Left <user message>`. The gateway persists + broadcasts. (Use `_fsc_port defaultFrontendConfig` for `<port>` — the closure is built earlier in the `let` than `feCfg`, and `feCfg` only overrides the bind host, not the port.)

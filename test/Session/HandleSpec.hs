  ...
  describe "validateRuntime" $ do
    ...
    it "SkHarness present in map returns RuntimeOk (TargetHarness name)" $ do
      let h = noOpHarness
          ...
          hSpec = HarnessSpec (fixedFlavourLookup "cc") (TmuxConfig "cc" "cc" Nothing) Nothing [] Nothing Nothing Nothing
    ...
    it "SkHarness absent returns RuntimeFallback TargetProvider with warning" $
      let hSpec = HarnessSpec (fixedFlavourLookup "dead") (TmuxConfig "dead" "dead" Nothing) Nothing [] Nothing Nothing Nothing

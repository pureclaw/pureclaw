...
spec = do
  ...
  describe "SessionMeta JSON (new format)" $ do
    ...
    it "round-trips a harness kind" $ do
      let s = sample
            { _sm_kind = SkHarness (HarnessSpec HClaudeCode (TbTmux (TmuxConfig "claude-code" "claude-code" Nothing)) Nothing [] Nothing)
            }
      Aeson.decode (Aeson.encode s) `shouldBe` Just s
    ...
    it "defaultTarget SkHarness uses flavour name as harness target" $
      defaultTarget (SkHarness (HarnessSpec HClaudeCode TbLocal Nothing [] Nothing))

...
spec = do
  ...
  describe "SessionMeta JSON (new format)" $ do
    ...
    it "round-trips a harness kind" $ do
      let s = sample
            { _sm_kind = SkHarness (HarnessSpec HClaudeCode (TmuxConfig "claude-code" "claude-code" Nothing) Nothing [] Nothing Nothing Nothing)
            }
      Aeson.decode (Aeson.encode s) `shouldBe` Just s
    ...
    it "defaultTarget SkHarness uses flavour name as harness target" $
      defaultTarget (SkHarness (HarnessSpec HClaudeCode (TmuxConfig "" "" Nothing) Nothing [] Nothing Nothing Nothing))

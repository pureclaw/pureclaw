...
**`cabal.project.freeze` deferred (decision at WU0 implementation time):** the round-2 plan called for a freeze file. The flake uses `haskell.nix` with `compiler-nix-name = ghc9123` (verified in `flake.nix:23`), which pins every transitive dependency via `flake.lock` the moment the flake is locked — `wai-websockets-3.0.1.2` is locked once the lock is in place, no separate freeze file required. Adding `cabal.project.freeze` would conflict with haskell.nix's resolved versions. The design's A06 requirement (pin for CVE review) is satisfied directly by `flake.lock`. If a future contributor switches off haskell.nix, a freeze file should be reintroduced.

**Validation gate:** `nix develop . --command cabal build all` succeeds with a non-vacuous `Stream.hs` that exercises `Network.Wai.Handler.WebSockets.websocketsOr` (the project's `-Werror` policy forbids vacuous imports, so a literal one-line `import + module` declaration would not compile). The WU0 `Stream.hs` is a **seed module** — it exports a `wu0SmokeTest :: Application -> Application` that wires `websocketsOr` to a `rejectRequest` server-app, proving the dependency is reachable end-to-end. WU3 expands `Stream.hs` to the full `streamApp` (the `wu0SmokeTest` export is replaced; see §WU3).
